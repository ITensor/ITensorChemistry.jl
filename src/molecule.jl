...
macro MoleculeName_str(s)
  return :(MoleculeName{$(Expr(:quote, Symbol(s)))})
end
...
Atom(name::String, x, y, z) = Atom(name, (x, y, z))

# generate an atom at r = (0,0,0)
Atom(name::String) = Atom(name, (0.0, 0.0, 0.0))
...
Molecule(atoms::Vector{<:Tuple}) = Molecule([Atom(a[1], a[2:4]) for a in atoms])

Molecule(atoms::Vector{<:Vector}) = Molecule(Tuple.(atoms))

Base.length(molecule::Molecule) = length(atoms(molecule))

Base.getindex(molecule::Molecule, i::Int) = atoms(molecule)[i]

Base.push!(molecule::Molecule, atom::Atom) = push!(molecule.atoms, atom)
